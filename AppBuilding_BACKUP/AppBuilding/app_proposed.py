...
if 'Quantity' in orders_df.columns:
    # Sum only Delivered and Shipped orders for sales volume, or all orders
...
with left_col:
    ...
    # Calculate recommended orders on the fly to match prediction logic
...
with chart_col1:
    if not forecast_df.empty and 'Month1' in forecast_df.columns:
        ...
        fig = px.bar(
            ...
            color_discrete_sequence=["#1E3A8A", "#3B82F6", "#60A5FA"] # HSL-derived blue theme
        )
        fig.update_layout(
